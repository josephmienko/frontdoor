...
from bridgewire.adapters.http.uvicorn_server import (
    ApiServer as ApiServerProtocol,
)
from bridgewire.adapters.http.uvicorn_server import (
    ApiServerLifecycleError,
    ApiServerShutdownTimeout,
    ApiServerSnapshot,
    ApiServerStartupCancelled,
...
@pytest.mark.unit
def test_uvicorn_preserves_immediate_post_start_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    failure = OSError("immediate failure")
    ...
    def hold_log(*_args: object, **_kwargs: object) -> None:
        logging_started.set()
        release_logging.wait(1)
    ...
    monkeypatch.setattr("bridgewire.adapters.http.uvicorn_server.uvicorn.Server", Server)
    monkeypatch.setattr("bridgewire.adapters.http.uvicorn_server.logger.error", hold_log)
    server = UvicornThreadServer(
        object(),  # type: ignore[arg-type]
        host="127.0.0.1",
        port=8080,
    ...
    try:
        with pytest.raises(ApiServerStartupError) as raised:
            server.start()
        assert logging_started.is_set()
        snapshot = server.snapshot()
        assert snapshot.state is ApiServerState.FAILED
        assert failure in snapshot.failures
        assert not isinstance(raised.value, ApiServerStartupTimeout)
    finally:
        release_logging.set()


@pytest.mark.unit
def test_uvicorn_logs_clean_runtime_exit_without_holding_lifecycle_lock(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    release = threading.Event()
    ...
    def probe_log(*_args: object, **_kwargs: object) -> None:
        nonlocal lock_was_available
        lock_was_available = server._lock.acquire(blocking=False)
        if lock_was_available:
            server._lock.release()
        logged.set()

    monkeypatch.setattr("bridgewire.adapters.http.uvicorn_server.logger.error", probe_log)
    server.start()
    release.set()
    assert logged.wait(1)
    assert lock_was_available
    assert server.snapshot().state is ApiServerState.FAILED


@pytest.mark.unit
def test_uvicorn_clean_early_exit_is_start_failure_and_single_use(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class Server:
        started = False
    ...
    with pytest.raises(ApiServerStartupError) as raised:
        server.start()
    assert not isinstance(raised.value, ApiServerStartupTimeout)
    assert server.snapshot().state is ApiServerState.START_FAILED
    server.stop()
    with pytest.raises(ApiServerLifecycleError):
        server.start()
...
@pytest.mark.unit
def test_uvicorn_startup_timeout_can_finish_its_initial_stop(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class Server:
        started = False
    ...
    with pytest.raises(ApiServerStartupTimeout):
        server.start()
    snapshot = server.snapshot()
    assert snapshot.state is ApiServerState.START_TIMED_OUT
...
@pytest.mark.unit
def test_uvicorn_stop_before_start_is_terminal_and_does_not_touch_server(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class Server:
        started = False
        ...
        def run(self) -> None:
            pytest.fail("thread must not start")
    ...
    server.stop()
    assert server.snapshot().state is ApiServerState.STOPPED
    assert not server._server.should_exit
    with pytest.raises(ApiServerLifecycleError):
        server.start()


@pytest.mark.unit
def test_uvicorn_overlapping_start_and_stop_is_typed_and_bounded(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert isinstance(failures[0], ApiServerStartupCancelled)
    assert server.snapshot().state is ApiServerState.STOPPED
    assert not server.snapshot().thread_alive


@pytest.mark.unit
def test_uvicorn_concurrent_stop_calls_share_one_join(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class Server:
        started = False
    ...
    assert CountingThread.join_calls == 1
    assert server.snapshot().state is ApiServerState.STOPPED


@pytest.mark.unit
def test_uvicorn_thread_construction_failure_is_synchronous(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    failure = OSError("thread construction failed")
    monkeypatch.setattr(
        "bridgewire.adapters.http.uvicorn_server.uvicorn.Config",
        lambda *_args, **_kwargs: object(),
    )
    monkeypatch.setattr(
        "bridgewire.adapters.http.uvicorn_server.uvicorn.Server",
        lambda _config: object(),
    )

    def fail_thread(**_kwargs: object) -> threading.Thread:
        raise failure
...
@pytest.mark.integration
@pytest.mark.parametrize(
    ("fail_start", "fail_stop", "fail_reader", "fail_audit"),
    [
...
def test_optional_api_failure_cannot_skip_processing_or_relay_cleanup(
    ...
    stop = threading.Event()
    caplog.set_level("ERROR", logger="bridgewire.hardware_service")
    ...
    class Session:
        ...
        def close(self) -> None:
            lifecycle.append("reader_close")
            if fail_reader:
                raise OSError("reader close failed")

    lifecycle: list[str] = []

    class TrackingHealth(FileHealthReporter):
        def report(self, status: str, **details: object) -> None:
            if status == "stopped":
                lifecycle.append("health_stopped")
            super().report(status, **details)

    class TrackingAudit(SQLiteAuditSink):
        def close(self) -> None:
            lifecycle.append("audit_close")
            super().close()
            if fail_audit:
                raise OSError("audit close failed")

    class TrackingRelay(RaspberryPiRelay):
        def cleanup(self) -> None:
            lifecycle.append("relay_cleanup_request")
            super().cleanup()

    monkeypatch.setattr("bridgewire.hardware_service.FileHealthReporter", TrackingHealth)
    monkeypatch.setattr("bridgewire.hardware_service.SQLiteAuditSink", TrackingAudit)
    monkeypatch.setattr("bridgewire.hardware_service.RaspberryPiRelay", TrackingRelay)
    ...
    assert "API" in repr(raised.value)
    if fail_reader:
        assert "reader close failed" in repr(raised.value)
    if fail_audit:
        assert "audit close failed" in repr(raised.value)
    assert ("output", 23, gpio.HIGH) in gpio.calls
    high_index = gpio.calls.index(("output", 23, gpio.HIGH))
    assert gpio.calls.index(("output", 23, gpio.LOW), high_index + 1) > high_index
    assert gpio.calls[-2:] == [("output", 23, gpio.LOW), ("cleanup", 23)]
    ...
    assert lifecycle.index("relay_low") < lifecycle.index("relay_cleanup_request")
    assert lifecycle.index("relay_cleanup_request") < lifecycle.index("reader_close")
    assert lifecycle.index("reader_close") < lifecycle.index("health_stopped")
    assert lifecycle.index("health_stopped") < lifecycle.index(
        "relay_cleanup_request", lifecycle.index("relay_cleanup_request") + 1
    )
    assert lifecycle.index("api_stop") < lifecycle.index("audit_close")
    assert (
        sum(record.message == "read-only API lifecycle failure" for record in caplog.records) == 1
    )
...
def test_complete_api_construction_is_isolated_from_hardware_processing(
    ...
    def fail_factory(_app: FastAPI, _host: str, _port: int) -> ApiServerProtocol:
        raise failure

    if target == "threading.Thread":
        monkeypatch.setattr(
            "bridgewire.adapters.http.uvicorn_server.threading.Thread",
            fail,
        )
    elif target != "api_server_factory":
    ...
    with pytest.raises(BaseException, match=target):
        run_hardware_service(
            ...
            install_signals=False,
            stop_event=stop,
            api_server_factory=fail_factory if target == "api_server_factory" else None,
        )
    assert ("output", 23, gpio.HIGH) in gpio.calls
    assert gpio.calls[-2:] == [("output", 23, gpio.LOW), ("cleanup", 23)]
